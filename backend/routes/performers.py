...
@performers_bp.route('/performers/<performer_id>/summary', methods=['GET'])
def get_performer_summary(performer_id):
    """
    Get performer summary (fast endpoint for initial page load).
    Returns performer metadata, instruments, and images - NO recordings.
    This allows the UI to render the performer profile quickly while
    recordings load in the background.
    """
    try:
        # Get performer information
        performer_query = """
        ...
        # Get total recording count (for display purposes, without fetching recordings)
        count_query = """
            SELECT COUNT(*) as count
            FROM recording_performers rp
            WHERE rp.performer_id = %s
        """
        count_result = db_tools.execute_query(count_query, (performer_id,), fetch_one=True)
        performer['recording_count'] = count_result['count'] if count_result else 0

        # No recordings in summary - those load separately
        performer['recordings'] = None

        return jsonify(performer)

    except Exception as e:
        logger.error(f"Error fetching performer summary: {e}")
        return jsonify({'error': 'Failed to fetch performer summary', 'detail': str(e)}), 500


@performers_bp.route('/performers/<performer_id>/recordings', methods=['GET'])
def get_performer_recordings(performer_id):
    """
    Get all recordings for a performer (heavier endpoint, call after summary loads).
    Supports sorting by 'year' (default) or 'name' (song title).
    """
    try:
        # Verify performer exists
        performer_check = db_tools.execute_query(
            "SELECT id FROM performers WHERE id = %s",
            (performer_id,),
            fetch_one=True
        )
        ...
        recordings_query = f"""
            ...
            ORDER BY {order_clause}
        """
        recordings = db_tools.execute_query(recordings_query, (performer_id,))
        ...
        logger.error(f"Error fetching performer recordings: {e}")
        return jsonify({'error': 'Failed to fetch performer recordings', 'detail': str(e)}), 500
